...
/**
 * @brief Imprime o timestamp atual (HH:MM:SS.mmm) sem quebra de linha.
 *
 * Usa clock_gettime(CLOCK_REALTIME) para precisão de milissegundos.
 */
...
/**
 * @brief Imprime o prefixo ANSI colorido correspondente à categoria.
 *
 * @param cat  Categoria do log.
 */
 ...
 *
 * @param flags  Byte de telemetria: bit i = sensor (i+1); 1=objeto, 0=livre.
 */
...
void log_fsm_state(int state_id)
{
    ...
    /* Pipeline visual — destaca o estado ativo entre colchetes */
    printf("          ");
...
void log_debug_menu(void)
{
    ...
    printf("  h  Executar Handshake\n");
    printf("  a  Envia destino A (apos reset -> handshake)\n");
    printf("  b  Envia destino B (apos reset -> handshake)\n");
