...
/**
 * @brief Extrai o byte de destino de rota a partir do texto de um QR code.
 *
 * Tenta duas estratégias em ordem:
 *   1. Procura o campo "Destino:" (ou "destino:") e lê o hex seguinte.
 *      Aceita formato "0xAADA" (byte alto = START_FRAME) ou "0xDA" isolado.
 *   2. Fallback: varre o texto inteiro procurando tokens "0xXX" válidos.
 *
 * @param text  Texto decodificado do QR (terminado em NUL). Pode ser NULL.
 * @return      ROUTE_A_SEND (0xDA), ROUTE_B_SEND (0xDB) ou 0 se não encontrado.
 */
static uint8_t parse_qr_text(const char *text)
{
    if (!text || text[0] == '\0') return 0;

    /* Estratégia 1: campo "Destino:" */
    ...
    /* Estratégia 2: varredura linear por tokens "0x.." */
    const char *p = text;
    while (*p) {
...
extern "C" CamResult camera_read_qr(int timeout_ms)
{
    ...
    while (1) {
        ...
        /* Converte para escala de cinza; o ZBar aceita apenas Y800 */
        cv::Mat gray;
